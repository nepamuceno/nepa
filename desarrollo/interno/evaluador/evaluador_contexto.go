...
// PrepararContextoEvaluador crea un contexto inicial con mapas vacíos
// y registra todas las funciones disponibles (matriz, estadística, conversiones, etc.)
func PrepararContextoEvaluador() *Contexto {
    ctx := &Contexto{
        Variables:  make(map[string]interface{}),
        Globales:   make(map[string]interface{}),
        Constantes: make(map[string]interface{}),
        Funciones:  make(map[string]func(...interface{}) interface{}),
    }

    // Aquí llamas a los registradores definidos en tus otros archivos
    RegistrarFuncionesMatriz(ctx)
    RegistrarFuncionesEstadistica(ctx)
    RegistrarFuncionesConversiones(ctx)
